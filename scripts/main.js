...
'use strict';
{
	...
	window.RuntimeInterface = class RuntimeInterface {
		...
		_ReportProjectMainScriptError(url, err) {
			...
			console.log(`Failed to load project main script (${url}). Check all your JavaScript code has valid syntax. Press F12 and check the console for error details.`)
		}
...

{
	...
	function addClassList(elem, firstClassName) {
		elem.classList.add(firstClassName);
	}
	function removeClassList(elem, firstClassName) {
		elem.classList.remove(firstClassName);
	}
	function replaceClassList(elem, firstClassName, secondClassName) {
		elem.classList.replace(firstClassName, secondClassName);
	}
	function toggleClassList(elem, firstClassName) {
		elem.classList.toggle(firstClassName);
	}
	...
	function changeClassList(action, elem, firstClassName, secondClassName) {
		if (action === "NONE")
			return;
		if (action === "ADD")
			addClassList(elem, firstClassName);
		if (action === "REMOVE")
			removeClassList(elem, firstClassName);
		if (action === "REPLACE")
			replaceClassList(elem, firstClassName, secondClassName);
		if (action === "TOGGLE")
			toggleClassList(elem, firstClassName);
	}
	...
	const HANDLER_CLASS = class InjectCSS_v2Handler extends self.DOMElementHandler {
		...
		UpdateState(elem, e) {
			...
			if (e["action"] === "ADD") {
				addLink(e);
				this.PostToRuntimeElement("changeaction", DOM_COMPONENT_ID, {
					"action": "NONE"
				});
			}
